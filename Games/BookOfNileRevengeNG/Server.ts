// Server.ts - BookOfNileRevengeNG game server
...
export class Server {
    ...
    private slotId: string = 'BookOfNileRevengeNG';
    ...
    private handleSpinRequest(postData: any, reqId: string): string {
            ...
            for (let k = 0; k < lines; k++) {
                for (let j = 0; j < this.slotSettings.SymbolGame.length; j++) {
                    ...
                    if (matchCount > 0 && this.slotSettings.Paytable['SYM_' + csym][matchCount] > 0) {
                        ...
                        if (wildMatches === matchCount && csym !== '9') mpl = 0;
            ...
            if (slotEvent === 'freespin') {
                ...
                for (const bSym of ExpandingSymbols) {
                    ...
                    if (this.slotSettings.Paytable['SYM_' + bSym][bSymCnt] > 0) {
                        const tmpWinArr = [];
                        const reelsEx = JSON.parse(JSON.stringify(reels));
                        for (let r = 1; r <= 5; r++) {
                            for (let p = 0; p <= 2; p++) {
                                if (reels[`reel${r}`]?.[p] == bSym) {
        ...
        if (slotEvent === 'freespin') {
            ...
            if (totalWin <= 0) {
                this.slotSettings.SetGameData(this.slotId + 'FreeGames', this.slotSettings.GetGameData(this.slotId + 'FreeGames') + 1);
            }
        } else {
        ...
        if (scattersCount >= 3) {
            if (this.slotSettings.GetGameData(this.slotId + 'pickCount') == 0) {
                this.slotSettings.SetGameData(this.slotId + 'pickCount', this.slotSettings.GetGameData(this.slotId + 'pickCount')); // Logic parity
            }
        ...
        if (totalWin > 0) {
            winString = `,"slotWin":{"lineWinAmounts":[${lineWins.join(',')}]${expWinStr},"totalWin":"${this.slotSettings.FormatFloat(totalWin)}","canGamble":"false"}`;
        } else {
            winString = '';
        }
    ...
    private getPaytable(): any[] {
        return [
            ...
            { "payout": "130", "symbols": ["10"], "type": "scatter" }
        ];
    }
