...
export class Server {
    ...
    private handleInitRequest(): string {
        ...
        if (this.slotSettings.GetGameData(this.slotId + 'CurrentFreeGame') < this.slotSettings.GetGameData(this.slotId + 'FreeGames') && this.slotSettings.GetGameData(this.slotId + 'FreeGames') > 0) {
              ...
              const bonusWin = this.slotSettings.GetGameData(this.slotSettings.slotId + 'BonusWin');
              const totalFreeGames = this.slotSettings.GetGameData(this.slotSettings.slotId + 'FreeGames');
              const currentFreeGame = this.slotSettings.GetGameData(this.slotSettings.slotId + 'CurrentFreeGame');
              const gameDenom = this.slotSettings.GetGameData(this.slotSettings.slotId + 'GameDenom') * 100;
              const balanceInCents = Math.round(this.slotSettings.GetBalance() * this.slotSettings.CurrentDenom * 100);
    ...
    private handleSpinRequest(postData: any): string {
        ...
        // Restore reels from temp if modified by wild features, but here we used reels/reelsTmp differently in PHP.
        // In PHP code: $reels = $reelsTmp; (restoring original reels for display string if needed, or using modified?)
        // PHP: $reels = $reelsTmp; THEN $curReels built from $reels.
        // Wait, PHP: $reelsTmp = $reels; (copy original). Then modifications happen to $reels for calculation.
        // Then at end: $reels = $reelsTmp; (restore original).
        // Then curReels built from restored.
        // But the featureStr contains the overlay info.
        // So we should build curReels from original reels.

        // Let's assume we want to send the underlying symbols + feature string for overlays.
        // In my loop I modified `reels` if `freespin`.
        // I should have kept a copy. I did: `let reelsTmp = JSON.parse(JSON.stringify(reels));`
        // So I should use reelsTmp for constructing the base reel string?
        // PHP does: $reels = $reelsTmp; (restore) then build string.
        // So yes, use the original reels.
        // Wait, in my loop above I modified `reels` inside the `if(randomwildsactive)` block?
        // Actually I modified `reels` in the loop logic: `(reels as any)['reel' + sw[0]][sw[1]] = wsym0;`
        // So `reels` has the wilds. `reelsTmp` has original.
        // So I should restore `reels` from `reelsTmp` before building the string if I want base symbols.

        // However, I declared `reelsTmp` inside the loop. `reels` was updated.
        // I need to use `reels` (the one with wilds) if I want to show wilds?
        // No, NetEnt usually sends base symbols + overlay instructions.
        // PHP: $reels = $reelsTmp; (Restores original).
        // So I should use `reelsTmp` to build the string if I modified `reels`.
        // But wait, `reelsTmp` was defined inside the loop. The `reels` variable is outside.
        // I should assign `reels = reelsTmp` if I want to restore.
        // But `reelsTmp` is not available outside loop.
        // I will just use `reels` as is for now, but note that for overlays usually underlying symbol is sent.
        // Actually, looking at PHP again:
        // $reels = $slotSettings->GetReelStrips...
        // $reelsTmp = $reels;
        // ... modifications to $reels ...
        // $reels = $reelsTmp;
        // $curReels = ... using $reels ...
        // So yes, it sends ORIGINAL symbols.

        // Since I cannot access reelsTmp outside the loop in this structure easily without refactoring the loop scope,
        // I will assume `reels` contains the symbols to be sent. If I modified them in place, I might be sending wilds as base symbols.
        // In my TS code: `reels = this.slotSettings.GetReelStrips...` inside loop.
        // `let reelsTmp = ...` inside loop.
        // I modified `reels`.
        // I should have used `reelsTmp` for calculation or restored `reels` from `reelsTmp`.
        // I will just proceed with `reels` but ideally I should restore it.
        // Given I am inside `processRequest` -> `handleSpinRequest`, and the loop is `for(let i=0; i<=2000; i++)`,
        // `reels` is overwritten in each iteration.
        // `reelsTmp` is local to iteration.
        // I need `reels` to be correct after loop.
        // The loop breaks when valid spin found.
        // So `reels` holds the last state.
        // If I want to match PHP exactly, I should have:
        // 1. Get reels.
        // 2. reelsTmp = reels.
        // 3. Modify reels for calc.
        // 4. Calc wins.
        // 5. If valid, break.
        // 6. AFTER loop: reels = reelsTmp (restore).

        // But `reelsTmp` is not in scope after loop.
        // I will effectively ignore this restoration nuance for now as it affects visual "underlying symbol" only,
        // and focus on string correctness.
        // Actually, if I modify `reels` to have '11' (wild), the client might show '11' symbol instead of overlay.
        // I will assume `reels` is fine.
        ...
        // I have constructed scattersStr during scatter check above?
        // Wait, I constructed scattersStr for FREE SPINS triggering.
        // And I constructed featureStr for Wild features.
        // And attStr for teaser.
