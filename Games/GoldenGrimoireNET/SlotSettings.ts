// SlotSettings.ts - GoldenGrimoireNET game specific settings
...
export class SlotSettings extends BaseSlotSettings {
    ...
    private initializeFromGameState(gameStateData: ISlotSettingsData): void {
        ...
        // Lines configuration
        // PHP defines 1..15, likely copied from FruitShopNET
        // The game Golden Grimoire usually has fixed 40 lines (or 20 lines sometimes?), but PHP code shows loop up to $lines=20.
        // And getLinesId in Server.php has 20 definitions.
        // So we should have 20 lines here to match logic.
    ...
    public GetRandomScatterPos(rp: string[] | number[]): number {
        ...
        if (rpResult.length === 0) {
            rpResult[0] = this.randomInt(2, rp.length - 4); // Adjusted to -4 based on GoldenGrimoire logic
        }
        ...
        if (winType !== 'bonus') {
            ...
            for (let index = 0; index < ['reelStrip1', 'reelStrip2', 'reelStrip3', 'reelStrip4', 'reelStrip5', 'reelStrip6'].length; index++) {
                ...
                if (Array.isArray(reelData) && reelData.length > 0) {
                    prs[index + 1] = this.randomInt(0, reelData.length - 4); // Adjusted -4
                }
            }
        } else {
            // Bonus spin - ensure scatters
            // In GoldenGrimoire PHP:
            // $scattersCnt = 5;
            // $reelsId = [1, 2, 3, 4, 5];
    ...
    private getLinesPercentConfig(type: string): { [key: string]: any } {
        // Default config from PHP if game data not present
        const defaultConfig = {
            'line1': { '1_100': 20 },
            'line3': { '1_100': 15 },
            'line5': { '1_100': 12 },
