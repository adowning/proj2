// SlotSettings.ts - GrandSpinnSuperpotNET game specific settings
...
export type ReelStrips = {
    ...
    [key: string]: Array<string | number> | number[] | number[][] | undefined;
};
...
export class SlotSettings extends BaseSlotSettings {
    ...
    private initializeFromGameState(gameStateData: ISlotSettingsData): void {
        ...
        this.Paytable = {
            ...
            'SYM_7': [0, 0, 0, 2, 0, 0],
            'SYM_8': [0, 0, 0, 1, 0, 0],
            'SYM_100': [0, 0, 0, 40, 0, 0],
            'SYM_101': [0, 0, 0, 200, 0, 0],
            'SYM_102': [0, 0, 0, 0, 0, 0],
            'SYM_50': [0, 0, 0, 0, 0, 0],
            'SYM_99': [0, 0, 0, 0, 0, 0]
        };
    ...
    public GetReelStrips(winType: string, slotEvent: string): ReelStrips {
        ...
        if (winType !== 'bonus') {
            ...
            for (let index = 0; index < ['reelStrip1', 'reelStrip2', 'reelStrip3', 'reelStrip4', 'reelStrip5', 'reelStrip6'].length; index++) {
                ...
                if (Array.isArray(reelData) && reelData.length > 0) {
                    prs[index + 1] = this.randomInt(3, reelData.length - 3); // PHP starts from 3
                }
            }
        } else {
            ...
            for (let index = 0; index < ['reelStrip1', 'reelStrip2', 'reelStrip3', 'reelStrip4', 'reelStrip5', 'reelStrip6'].length; index++) {
                ...
                const reelData = (this as any)[reelStrip];
                if (Array.isArray(reelData) && reelData.length > 0) {
                    prs[index + 1] = this.GetRandomScatterPos(reelData);
        ...
        for (const [index, value] of Object.entries(prs)) {
            ...
            if (key && key.length > 0) {
                ...
                // PHP sets 'rps' array for GrandSpinnSuperpotNET
                if (!reel.rps) reel.rps = [];
                if (!reel.rps[reelIndex - 1]) reel.rps[reelIndex - 1] = [];
                reel.rps[reelIndex - 1] = [value, value + 1, value + 2];
            }
        }
