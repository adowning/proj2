// SlotSettings.ts - VikingsNET game specific settings
...
export class SlotSettings extends BaseSlotSettings {
    ...
    private initializeFromGameState(gameStateData: ISlotSettingsData): void {
        ...
        this.Paytable = {
            'SYM_0': [0, 0, 0, 0, 0, 0, 0, 0],
            'SYM_1': [0, 0, 0, 0, 0, 0, 0, 0],
            'SYM_2': [0, 0, 0, 0, 0, 0, 0, 0],
            'SYM_3': [0, 0, 0, 30, 40, 50, 60, 70],
            'SYM_4': [0, 0, 0, 6, 10, 20, 24, 28],
            'SYM_5': [0, 0, 0, 6, 10, 20, 24, 28],
            'SYM_6': [0, 0, 0, 6, 10, 20, 24, 28],
            'SYM_7': [0, 0, 0, 6, 10, 20, 24, 28],
            'SYM_8': [0, 0, 0, 2, 4, 10, 16, 20],
            'SYM_9': [0, 0, 0, 2, 4, 10, 16, 20],
            'SYM_10': [0, 0, 0, 2, 4, 10, 16, 20],
            'SYM_11': [0, 0, 0, 2, 4, 10, 16, 20],
            'SYM_12': [0, 0, 0, 2, 4, 10, 16, 20],
            'SCATTER_PAYS': [
                0, 0, 0, 6, 10, 20, 40, 80, 160, 320, 640, 960, 1440, 2160, 3240,
                4860, 6000, 7000, 14000, 28000, 40000, 60000, 80000, 110000, 150000,
        ...
        this.slotWildMpl = 1;
        this.GambleType = 1;
        this.slotFreeCount = [0, 0, 0, 7, 7, 7];
        this.slotFreeMpl = 1;
    ...
    public GetBonusReelStrips(): ReelStrips {
        // Fetch reel strips for bonus
        const reelStripsBonus = this.gameDataStatic?.reelStripsBonus || {};
        const reelKeys = ['reelStrip1', 'reelStrip2', 'reelStrip3', 'reelStrip4', 'reelStrip5', 'reelStrip6', 'reelStrip7'];

        // This part in PHP relies on array_shift logic if reelStripsBonus is an array.
        // Assuming reelStripsBonus structure mirrors what PHP expects or uses direct key assignment
        ...
        for (const [index, reelStrip] of reelKeys.entries()) {
            const reelData = (this as any)[reelStrip];
            if (Array.isArray(reelData) && reelData.length > 0) {
                prs[index + 1] = this.randomInt(0, reelData.length - 7);
            }
        }
        ...
        for (const [index, value] of Object.entries(prs)) {
            ...
            if (key && key.length > 0) {
                ...
                const v_plus_3 = value + 3 >= cnt ? key[value + 3 - cnt] : key[value + 3];
                const v_plus_4 = value + 4 >= cnt ? key[value + 4 - cnt] : key[value + 4];
                const v_plus_5 = value + 5 >= cnt ? key[value + 5 - cnt] : key[value + 5];
    ...
    public GetReelStrips(winType: string, slotEvent: string): ReelStrips {
        ...
        if (winType !== 'bonus') {
            ...
            for (let i = 0; i < reelsId.length; i++) {
                if (i >= 2) { // Logic from PHP: if ($i >= 2) -> reels 3, 4, 5
