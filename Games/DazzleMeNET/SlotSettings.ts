// SlotSettings.ts - DazzleMeNET game specific settings
...
export class SlotSettings extends BaseSlotSettings {
    ...
    private initializeFromGameState(gameStateData: ISlotSettingsData): void {
        ...
        // Paytable configuration for DazzleMeNET
        this.Paytable = {
            'SYM_0': [0, 0, 0, 0, 0, 0],
            'SYM_1': [0, 0, 0, 0, 0, 0],
            'SYM_2': [0, 0, 0, 0, 0, 0],
        ...
        this.slotWildMpl = 1;
        this.GambleType = 1;
        this.slotFreeCount = [0, 0, 0, 8, 12, 16]; // Scatter count based free spins
        this.slotFreeMpl = 1;
        ...
        // Lines configuration - DazzleMeNET uses 20 paylines
        ...
        this.Bet = this.game?.bet ? this.game.bet.split(',').map(b => parseFloat(b.trim())) : [0.01, 0.02, 0.05, 0.1, 0.2, 0.5, 1.0, 2.0];
    ...
    public GetRandomScatterPos(rp: string[]): number {
        ...
        for (let i = 0; i < rp.length; i++) {
            if (rp[i] === '0') { // Symbol 0 is the scatter
                if (rp[i + 1] && rp[i - 1]) {
                    rpResult.push(i);
                }
                if (rp[i - 1] && rp[i - 2]) {
    ...
    public GetReelStrips(winType: string, slotEvent: string): ReelStrips {
        ...
        for (const [index, value] of Object.entries(prs)) {
            ...
            if (key && key.length > 0) {
                ...
                // DazzleMeNET specific reel configuration
                if (reelIndex === 1 || reelIndex === 2) {
                ...
                } else if (reelIndex === 3 || reelIndex === 4) {
                    reel[`reel${reelIndex}`] = [
                        v_minus_1,
                        key[value],
                        v_plus_1,
                    ...
                    reel[`reel${reelIndex}`] = [
                        ...
                        v_plus_1,
                        v_plus_2,
                        key[value + 3 >= cnt ? value + 3 - cnt : value + 3],
                        ''
                    ];
                }
    ...
    private getLinesPercentConfig(type: string): { [key: string]: any } {
        const defaultConfig = {
            ...
            'line15_bonus': { '1_100': 30 },
            'line20_bonus': { '1_100': 25 }
        };
