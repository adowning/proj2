// SlotSettings.ts - WingsOfRichesNET game specific settings
...
export class SlotSettings extends BaseSlotSettings {
    ...
    private initializeFromGameState(gameStateData: ISlotSettingsData): void {
        ...
        this.Paytable = {
            ...
            'SYM_1': [0, 0, 0, 5, 20, 75],
            'SYM_2': [0, 0, 0, 0, 0, 0],
            'SYM_3': [0, 0, 0, 40, 150, 1500],
            'SYM_4': [0, 0, 0, 30, 125, 1200],
            'SYM_5': [0, 0, 0, 25, 100, 700],
            'SYM_6': [0, 0, 0, 20, 75, 500],
            'SYM_7': [0, 0, 0, 10, 50, 150],
            'SYM_8': [0, 0, 0, 8, 30, 125],
            'SYM_9': [0, 0, 0, 6, 25, 100],
            'SYM_11': [0, 0, 0, 4, 15, 50],
            'SYM_12': [0, 0, 0, 3, 10, 25]
        };
    ...
    public GetReelStrips(winType: string, slotEvent: string): ReelStrips {
        ...
        if (slotEvent === 'freespin') {
            const reelStripsBonus = this.gameDataStatic?.reelStripsBonus;
             if (reelStripsBonus) {
                // Assuming reelStripsBonus is an array of arrays or object with keys
                 // PHP: $fArr = $reel->reelsStripBonus; $curReel = array_shift($fArr);
                 // We will need to see how reelsStripBonus is structured in JS gameDataStatic.
                 // Assuming it follows similar structure to reelStrips.
                 // However, PHP logic seems to overwrite the main reel strips with bonus ones.
                 // We should probably check if `reelStripsBonus` exists on this object or needs to be fetched from gameDataStatic.

                 // Since we don't have GameReel class here, we rely on gameDataStatic having `reelStripsBonus`.
                 // If not present, we fall back to normal strips.

                 // NOTE: The PHP code shifts from an array. It implies `reelsStripBonus` is an array of arrays?
                 // Or an object with keys?
                 // Based on `array_shift`, it's likely an array of arrays where index 0 is reel1, etc.

                 // Let's attempt to load it if available.
                 if (this.gameDataStatic.reelStripsBonus) {
                     const fArr = Object.values(this.gameDataStatic.reelStripsBonus); // Convert to array if object
                     // Map to reelStrip1..6
                      for (let i = 0; i < 6; i++) {
                         if (fArr[i]) {
                             (this as any)[`reelStrip${i + 1}`] = fArr[i];
                         }
                     }
                 }
            }
