    ...
    private initializeFromGameState(gameStateData: ISlotSettingsData): void {
        ...
        if (this.game) {
            for (let jp = 1; jp <= 4; jp++) {
                // Access dynamic properties manually if needed, or rely on Game model structure
                const jpVal = (this.game as any)[`jp_${jp}`];
                const jpPercent = (this.game as any)[`jp_${jp}_percent`];
                if (jpVal !== undefined) this.slotJackpot.push(jpVal);
                if (jpPercent !== undefined) this.slotJackPercent.push(jpPercent);
            }
        }
