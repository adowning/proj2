// SlotSettings.ts - JumanjiNET game specific settings
...
export class SlotSettings extends BaseSlotSettings {
    ...
    private initializeFromGameState(gameStateData: ISlotSettingsData): void {
        ...
        this.slotFreeCount = [0, 0, 0, 0, 0, 0];
        this.slotFreeMpl = 1;
    ...
    public GetReelStrips(winType: string, slotEvent: string): ReelStrips {
        if (slotEvent === 'freespin') {
            // Jumanji has special handling for freespins reel strips in PHP logic (copying from reelsStripBonus)
            // Assuming loaded or handled similarly to previous game
        }
        ...
        for (const [index, value] of Object.entries(prs)) {
            ...
            if (key && key.length > 0) {
                // Jumanji reel shape is irregular (3-4-5-4-3)
                // PHP code:
                /*
                if( $index == 1 || $index == 5 ) { ... 3 symbols ... }
                if( $index == 2 || $index == 4 ) { ... 4 symbols ... }
                if( $index == 3 ) { ... 5 symbols ... }
                */
                // Wait, the PHP code actually assigns ALL symbols up to key+3 or more, but only based on index?
                // The PHP code:
                /*
                if( $index == 1 || $index == 5 ) {
                ...
                */
                // It seems it fills up empty slots for alignment.
                // Note: The wrapping logic $key[-1], etc., is used in PHP.
                // We need to simulate that.

                const cnt = key.length;
                const v_minus_1 = value - 1 < 0 ? key[cnt - 1] : key[value - 1];
                const v_0 = key[value];
                const v_plus_1 = value + 1 >= cnt ? key[value + 1 - cnt] : key[value + 1];
                ...
                if (reelIndex === 1 || reelIndex === 5) {
                    reel[`reel${reelIndex}`]!.push(v_minus_1);
                    reel[`reel${reelIndex}`]!.push(v_0);
                    reel[`reel${reelIndex}`]!.push(v_plus_1);
                    reel[`reel${reelIndex}`]!.push('');
                    reel[`reel${reelIndex}`]!.push('');
                    reel[`reel${reelIndex}`]!.push('');
                }
                if (reelIndex === 2 || reelIndex === 4) {
                    reel[`reel${reelIndex}`]!.push(v_minus_1);
                    reel[`reel${reelIndex}`]!.push(v_0);
                    reel[`reel${reelIndex}`]!.push(v_plus_1);
                    reel[`reel${reelIndex}`]!.push(v_plus_2);
                    reel[`reel${reelIndex}`]!.push('');
                    reel[`reel${reelIndex}`]!.push('');
                }
                if (reelIndex === 3) {
                    ...
                    reel[`reel${reelIndex}`]!.push(v_plus_1);
                    reel[`reel${reelIndex}`]!.push(v_plus_2);
                    reel[`reel${reelIndex}`]!.push(v_plus_3);
                    reel[`reel${reelIndex}`]!.push('');
                }
