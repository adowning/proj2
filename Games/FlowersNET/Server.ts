...
export class Server {
    ...
    private handleSpinRequest(postData: any): string {
        ...
        let freeState = '';
        const balanceInCents = Math.round(this.slotSettings.GetBalance() * this.slotSettings.CurrentDenom * 100);
        let scattersCount = 0; // Needed for scope in free spin check below logic if reused

        // Re-calculate scatters count for final response logic
        let finalScattersCount = 0;
        for (let r = 1; r <= 5; r++) {
            for (let p = 0; p <= 2; p++) {
                const rKey = `reel${r}`;
                const val = (reels[rKey] as any[])?.[p];
                if (String(val) === '0') finalScattersCount++;
                if (String(val) === '12') finalScattersCount += 2;
            }
        }

        if (finalScattersCount >= 4) {
            this.slotSettings.SetGameData('FlowersNETFreeStartWin', totalWin);
            this.slotSettings.SetGameData('FlowersNETBonusWin', totalWin);
            this.slotSettings.SetGameData('FlowersNETFreeGames', this.slotSettings.slotFreeCount[finalScattersCount] || 0);
